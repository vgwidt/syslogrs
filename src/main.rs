...
fn main() -> Result<(), Error> {
    ...
    poll.registry().register(&mut udp4_server_mio, SERVER, Interest::READABLE)?;
    let mut shutdown = false;
    while !shutdown {
        poll.poll(&mut events, None)?; //Poll for events
        for event in events.iter() {
            match event.token() { //Check for event token
                SERVER => match receive(&udp4_server_mio, &mut buffer) { //Receive UDP packet
                    Ok(()) => {                        
                        continue
                    }
                    Err(e) => {
...
fn receive(sock: &UdpSocket, buf: &mut [u8]) -> Result<(), Error> {
    loop {
        ...
        let (len, from) = match sock.recv_from(buf) {
            ...
            Err(e) => {
                if e.kind() == ErrorKind::WouldBlock || e.kind() == ErrorKind::Interrupted { //If would block, return
                    return Ok(());
                } else {
                    return Err(e);
                }
        ...
        println!("[{}] {}", from, s);
        for elem in buf.iter_mut() { *elem = 0; } //reset array to clear buffer
